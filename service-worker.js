...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(CACHE).then(function (cache) {
      ...
      cache.addAll(cacheList).catch(e=>{})
   ...
   function(){
    ...
    if (cachedResponse) {
      resolve(cachedResponse)
                fetch(event.request).then(function(responseData){
                  console.log("[PWA Builder] update cache: " + response.url);
        ...
        fetch(event.request).then(function(responseData){
                  console.log("[PWA Builder] add page to cache: " + response.url);
