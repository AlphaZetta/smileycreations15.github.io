...
function smileycreations15_api() {
    ...
    smileycreations15_prototype.dialogBox = function dialogBox(location = "top-left", type = "plain", dialogContent, black = true) {
        let dialog = document.createElement("div") dialog.className = "notify " + location + " do-show font-notify"
        dialog.dataset.notificationStatus = type dialog.innerHTML = dialogContent // positions : bottom-right, top-left, top-right, bar-bottom, bar-top, bottom-right, bottom-left
    ...
    smileycreations15_prototype.showLoaderOverlay = function showLoaderOverlay(id, text = null, overlayHtml = false) {
        ...
        let div = document.createElement("div") let option = "center"
        ...
        document.body.appendChild(div) var proto = {
...
window.smileycreations15 = smileycreations15_api() var deferredPrompt = {
    prompt: (() => {}
...
function installPWA() {
    ...
    ui = smileycreations15.showLoaderOverlay("ui-install", "Please wait...") ui.show() deferredPrompt.prompt() deferredPrompt.userChoice.then((choiceResult) => {
            ui.remove() if (choiceResult.outcome === 'accepted') {
                ui = smileycreations15.showLoaderOverlay("ui-install", "Installation in progress...<br><span style='font-size:20px'>Please do not close the tab.</span>") ui.show() window.addEventListener('appinstalled', (evt) => {
                        postSecure({
                                "action": "pwaStatus",
                        ...
                        ui.remove() smileycreations15.dialogBox("top-left", "success", "App installed successfully")
                    }
...
window.addEventListener('beforeinstallprompt', (e) => {
        ...
        let b = document.createElement("a") b.onclick = installPWA b.id = "installPrompt"
