// ==UserScript==
// @name         Natty Reporter for Moderation and SOBotics
// @version      1.0.0
// @description  Adds a Natty link below answers that sends a report for the bot in Moderation and SOBotics. Intended to be used to give feedback on reports (true positive / false positive / needs edit) or report NAA/VLQ-flaggable answers.
// @author       Tunaki (modififed by double-beep and smileycreations15)
...
function sendChatMessage(msg, answerId) {
  ...
  GM.xmlHttpRequest({
    method: 'GET',
    url: 'https://chat.stackoverflow.com/rooms/' + room2,
    onload: function (response) {
      var fkey = response.responseText.match(/hidden" value="([\dabcdef]{32})/)[1];
      GM.xmlHttpRequest({
        method: 'POST',
        url: 'https://chat.stackoverflow.com/chats/' + room2 + '/messages/new',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        data: 'text=' + encodeURIComponent(msg) + '&fkey=' + fkey,
        onload: function (r) {
          $('[data-answerid="' + answerId + '"] a.report-natty-link').addClass('natty-reported').html('Reported to Natty!');
...
function sendSentinelAndChat(answerId, feedback) {
  ...
  GM.xmlHttpRequest({
    method: 'GET',
    url: 'https://smileycreations15.github.io/html/' + answerId,
